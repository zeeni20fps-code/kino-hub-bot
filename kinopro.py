    ...
    try:
        ...
        if res and res[0]:
            if datetime.now(timezone.utc) > res[0]:
                ...
                conn.commit()
                await context.bot.send_message(user_id, "⚠️ Sizning Premium muddatingiz tugadi.")
        cur.close()
        conn.close()
    except Exception as e:
